...
class orderDelete extends React.Component {
  ...
  offVisible = () =>{
    ...
    for (i=0;i<data.length;i++){
      data[i].保单操作 = <div> <Button onClick={this.orderDelete.bind()}>保单删除</Button> </div>
      if(text == data[i].订单号)
      {
        console.log("根据订单号找到了数据")
        find_data[0] = data[i];
        flag = 1;
      }
    }
    ...
    console.log("find_data数据样式",data);
    ...
    return (
      <div>
        ...
        <div style={{display:this.state.display_con}} className={styles.findOrder} >
          ...
          <br/>
          <br/>
          <Button onClick={this.orderDelete.bind()}>保单删除</Button>
